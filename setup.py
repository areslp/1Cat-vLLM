...
class CMakeExtension(Extension):
    def __init__(
        ...
        **kwa,
        ...
        # Targets that CMakeLists.txt deliberately builds against the full
        # CPython ABI (WITH_SOABI, no USE_SABI) must pass py_limited_api=False:
        # CMake names those `.cpython-<ver>-<plat>.so`, and build_ext otherwise
        # goes looking for an `.abi3.so` that is never produced.
...
if _is_cuda():
    if _cuda_arch_contains(7, 0):
        ...
        # The H3 / exact-reduce / sparse-attention kernels are defined with
        # WITH_SOABI and no USE_SABI, i.e. ordinary Torch/Python ABI, so they
        # must not be declared limited-API here.
        for _sm70_soabi_ext in (
            ...
            ext_modules.append(
                CMakeExtension(name=_sm70_soabi_ext, py_limited_api=False)
            )
